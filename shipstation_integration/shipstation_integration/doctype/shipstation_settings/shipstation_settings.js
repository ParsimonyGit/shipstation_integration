...
function set_item_field_options(frm) {
	...
	frappe.model.with_doctype("Sales Order Item", () => {
		...
		// sort fields alphanumerically by label
		options = options.sort((a, b) => {
			return a.label.localeCompare(b.label);
		});

		// update the individual row's "item_field" field
		frm.fields_dict.shipstation_options.grid.update_docfield_property(
			"item_field",
			"options",
			options
		);
...
frappe.ui.form.on("Shipstation Item Custom Field", {
	before_item_custom_fields_remove: (frm, cdt, cdn) => {
		...
		// check if there are rows in the options table that have the deleted field
		const deleted_row_in_use =
			frm.doc.shipstation_options.find((row) => {
		...
		if (deleted_row_in_use && deleted_row.fieldname) {
			frappe.throw(
				__(
					`Cannot delete field ${deleted_row.label.bold()} because it is being used in the Options table.`
				)
			);
		} else {
